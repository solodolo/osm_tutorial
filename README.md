...
This is a tutorial for building a map of Tanzania using OSM data. [This tutorial](https://blog.rustprooflabs.com/2019/01/postgis-osm-load) was used as a starting point and tweaked for current versions of all the tools.
...
1. Set up Postgres with PostGIS extension.
    ...
    ```
1. Install osm2pgsql. This will allow us to import the osm.pbf files into our postgis database. First we need to grab a "style" to tell osm2pgsql how the data should be formatted on import. Read more about styles [here](https://osm2pgsql.org/doc/manual.html#the-flex-output).
    1. Grab the default lua style from the osm2pgsql repository.
